-- Scoring v3: +2 per sighting of an untagged dog (welfare incentive).
--
-- Rationale: tagged dogs already attract a +2 ear-tag bonus when the
-- spotter photographs the tag. Untagged dogs have no equivalent reward
-- yet they're harder to identify across sightings and arguably need
-- more attention. A symmetric +2 bonus levels the incentive.
--
-- Untagged is defined at the *dog* level — dogs.ear_tag_id IS NULL —
-- not at the sighting level, so naming or tagging a dog later doesn't
-- retroactively void the bonus on prior sightings (that would feel
-- punitive). At the moment of spotting, was the dog tagged?
--
...
BEGIN
    ...
    -- "Untagged" reads the current dogs.ear_tag_id (if it gets set later
    -- the historical sightings still count — the bonus reflects the
    -- effort of cataloguing an unidentified dog).
